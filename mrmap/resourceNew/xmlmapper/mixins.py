            ...
            try:
                if not (isinstance(self._fields.get(key), xmlmap.NodeField) or
                    ...
                    if isinstance(self._fields.get(key), xmlmap.SimpleBooleanField) and getattr(self, key) is None or \
                       isinstance(self._fields.get(key), xmlmap.IntegerField) and getattr(self, key) is None or \
                       isinstance(self._fields.get(key), xmlmap.StringField) and getattr(self, key) is None:
