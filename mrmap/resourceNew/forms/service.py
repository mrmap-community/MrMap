...
from resourceNew.models import OgcService, Layer, FeatureType, OgcWms
...
class CreateOgcWmsModelForm(ModelForm):
    owned_by_org = forms.ModelChoiceField(
        label=_("Registration for organization"),
        help_text=_("Select for which organization you'd like to register the service."),
        queryset=Organization.objects.none(),
        to_field_name='id',
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if not self.request.user.is_anonymous:
            self.fields['owned_by_org'].queryset = self.request.user.get_publishable_organizations().\
                filter(is_autogenerated=False)
